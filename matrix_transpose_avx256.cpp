...
void transpose_naive(size_t columns, size_t rows, const int8_t *__restrict pSrc, ptrdiff_t srcLineStride, int8_t *__restrict pDst,
                     ptrdiff_t dstLineStride) noexcept
{
...
// original https://qiita.com/beru/items/12b4249c95a012a28ccd
inline void transpose(size_t width, size_t height, const int8_t *__restrict pSrc, ptrdiff_t srcLineStride,
                      int8_t *__restrict pDst, ptrdiff_t dstLineStride) noexcept
{
    ...
    if (width >= 16 / sizeof(int8_t) && height >= 16)
    {
        size_t bw = std::min<size_t>(128u / sizeof(int8_t), flp2(width));
        size_t bh = std::min<size_t>(512u / sizeof(int8_t), flp2(height));
        size_t nw = width / bw;
        size_t nh = height / bh;
...
int main()
{
    ...
    const size_t padding = 128;
    // const size_t rep = 7;
    // const size_t bufferSize = (height * std::pow(2, rep) + padding) * (height * std::pow(2, rep) + padding);
    size_t srcLineStride = width + padding;
    size_t dstLineStride = height + padding;
    const int32_t align_number = 64;
    int8_t *a = (int8_t *)aligned_alloc(align_number, width * height);
    int8_t *trans_a = (int8_t *)aligned_alloc(align_number, width * height);
    transpose(width, height, a, srcLineStride, trans_a, dstLineStride);
